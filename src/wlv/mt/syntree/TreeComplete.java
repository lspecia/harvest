 ...
 * This class is used for processing syntactic treess included in the GALE Phase 4 Arabic Parallel Aligned Treebank Part 1 Version 1.1 (LDC2009E82) dataset
 * Syntactic trees are in the penn treebank format and contain indeces to tokens. This class produces similar trees but replaces indeces with the referenced tokens
 *  * Usage: wlv.mt.syntree.TreeComplete [treeFileFolder] [tokenFileFolder] [outputFolder]
 */
...
/**
 * Reads in a folder of files containing syntactic trees and produces a similar file where the indeces are replaced with tokens
 ...
 * @param outFolder the output folder where the new complete trees will be stored
 */
				...
				/*	if (tag.trim().equals("-NONE-")){
						...
						try {
							...
							if (tag.equals("-RRB-"))
								...
								tWord = "-LRB-";
							}catch (java.lang.NumberFormatException e){e.printStackTrace();tWord = tokens.get(new Integer(16));}
						
						if (tWord==null)
						  System.out.println("null token "+token+"\t"+inTreeFile+"\t"+treeLine);
						} catch (Exception e ){e.printStackTrace();System.out.println(inTreeFile+"\t"+treeLine);}
					}
					if (!deleted && isToken){
...
//				ich = brTree.read();
	...
	/**
	 * Checks whether the tag has to be replaced in order to make it compliant with the Stanford parser output
	 * 
	 * @param tag 
	 * @return the replacement tag if tag needs to be replaced, or tag if no changes are necessary
	 */
	...
	/**
	 * Extracts the tokens from a line in a tokenised file and stores them in a HashMap in the format [tokenIndex,Token>]<br>
	 * The tokenised file is part of the GALE Phase 4 Arabic Parallel Aligned Treebank Part 1 Version 1.1 (LDC2009E82)
	 * For Arabic, the token is the unvocalized Buckwalter form of the word 
