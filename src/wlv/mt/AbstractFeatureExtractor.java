...
/**
 *  AbstractFeatureExtractor is the base class for any FeatureExtractor. It provides implementations for the methods that 
 *   create the folder structure of the projec and definitions for methods that have to be implemented b subclasses
 *         
 *           
...
public abstract class AbstractFeatureExtractor {
	...
	/**
	 * The implementations of this abstract method should handle the command line arguments and retrieve parameters required by the application
	 * Since the command line arguments are application specific, this method doesn't have a default implementation  
	 *
	...
	/**
	 * Performs some basic processing of the input source and target files. 
	 * The input files are also copied to the /input folder. This is necessary because the
	 * some tools (e.g., the MADA morphological analyser) produce their output in the same folder as the input file,
	 ...
	 * Subclasses can override this method to construct, for example, subfolders of the systems folder for each available MT system, or temporary folders
	 */
