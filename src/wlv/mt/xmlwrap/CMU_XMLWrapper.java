...
/**
 * This class produces an XML file from the output of the CMU machine translation system
 * It also produces a series of text files from the nbest list (one file per sentence), which will be used
 * in computing the glass box features that rely on ngrams computed on the nbest list 
 * 
 */

public class CMU_XMLWrapper implements XMLWrapper{
	...
	public CMU_XMLWrapper(String inputFile, String outputFile, String phraseFile, String logFile){
		inFile = inputFile;
		outFile = outputFile;
		bestScore = 0;
//		this.nbestPath = nbestPath;
		File f = new File(outFile);
		Logger.log("CMU_XMLWrapper running ...");
		Logger.log("nbest input file: "+inputFile);
		Logger.log("1-best phrases file: "+phraseFile);
		
		if (f.exists()) {
			Logger.log("Output file "+outputFile+" already exists. CMU XMLWrapper will not run.");
			System.out.println("Output file "+outputFile+" already exists. CMU XMLWrapper will not run.");
		}
		else {
		try {
	...
	public void registerResources(){
		for (String res:featName)
			wlv.mt.tools.ResourceManager.registerResource(res);
	}
...
//				System.out.println(phrases[i+1]+" span length="+span.length);
		...
		}catch (Exception e){e.printStackTrace();Logger.log(e.getStackTrace().toString());}
	}
...
//          (OutputKeys.OMIT_XML_DECLARATION, "yes");
	...
	public void run(){
		File f = new File(outFile);
		if (f.exists()) {
			Logger.log("Output file "+outFile+" already exists. CMU XMLWrapper will not run.");
			return;
		}
...
/*		for (int i=0; i<count;i++){
		...
		Logger.log("CMU XMLWrapper done!");
	}
...
//		System.out.println(trans.first().getAttribute("text"));
	...
	public static void main(String[] args){
		CMU_XMLWrapper cmu = new CMU_XMLWrapper(args[0], args[1], args[2], args[3]);
		cmu.run();
