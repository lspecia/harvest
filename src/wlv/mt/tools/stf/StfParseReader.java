 ...
 *
 * Reads in a file containing Stanford parser dependencies in the format produced by @see StfDepParserWrapper
 */
public class StfParseReader {
	...
	/**
	 * Retrieves the parse tree and the dependencies corresponding to the following sentence in the input and processes it
	 * It retrieves a series of values from the parsed input and sets them as values of the Sentence parameter
	...
	public void processNextSentence(Sentence sent){
		...
		String[] tokens;
		wlv.mt.util.Pair p=null;
		while (!lineTrim.isEmpty()&&line!=null){
			tokens = lineTrim.substring(4,lineTrim.length()-1).split(",");
			if (tokens.length==2){
			p = new wlv.mt.util.Pair(tokens[0].substring(0,tokens[0].lastIndexOf("-")),tokens[1].substring(0,tokens[1].lastIndexOf("-")));
	...
	/**
	 * Processes the parse tree and sets the phrase count for each type of phrase as values of the sentence parameter  
